# Odari K. Charles  
**Electrical & Electronics Engineer | Embedded Systems | IoT & Backend Engineering**


[![GitHub](https://img.shields.io/badge/GitHub-ODARI--CHARLES1-181717?style=for-the-badge&logo=github)](https://github.com/ODARI-CHARLES1)
[![Portfolio](https://img.shields.io/badge/Portfolio-View-blue?style=for-the-badge&logo=google-chrome)](https://charles.k.odari.portfolio.thegtm.or.ke/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin)](https://ke.linkedin.com/in/odari-kibisi-charles-329b19331)
[![Email](https://img.shields.io/badge/Email-Contact-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:daymondodari68@gmail.com)

---

I am an Electrical and Electronics Engineering student focused on designing and building end-to-end engineering systems that integrate hardware, firmware, and backend software. My interests include IoT systems, Robotics, embedded development, Artificial Intelligence(AI), backend services, and engineering simulations, supported by a strong foundation in mathematics and system modeling.

I work on practical engineering problems where reliability, efficiency, and scalability are critical.

---

## Core Areas of Expertise

### Embedded Systems and IoT
- Microcontroller programming (ESP32, Arduino)
- Sensor interfacing and data acquisition
- Embedded C/C++ development
- PCB design and circuit simulation
- Real-time and low-power system design

### Backend and Software Engineering
- Backend development with Python (Django, Flask)
- Node.js and RESTful API design
- System integration for IoT platforms
- SQL database design and management

### Engineering and Scientific Computing
- MATLAB and Simulink for modeling and analysis
- Circuit simulation and verification
- Numerical methods and applied mathematics
- System-level simulation and testing

---

## Technical Stack

### Programming Languages
- C, C++
- Python
- JavaScript, TypeScript
- Java
- Bash

### Frameworks and Tools
- Django, Flask, Express.js
- React (system dashboards and interfaces)
- Git, Linux, Docker
- KiCAD, Proteus
- MATLAB, LTspice, Multisim

### Databases
- MySQL
- PostgreSQL
- SQLite

---

## Selected Projects

### IoT Smart Monitoring System
Designed and implemented a real-time IoT monitoring system for remote sensor and health data collection.

**Key components**
- ESP32-based data acquisition
- Sensor integration and signal conditioning
- Backend services for data storage and access
- REST API for system communication

**Technologies**  
ESP32, Embedded C/C++, Python, Django, SQL

---

### Scientific and Mathematical Computing Tools
Development of computational tools and mathematical models for engineering analysis and simulation.

**Technologies**  
Python, MATLAB, numerical methods

---

### Backend Systems for Data Processing
Backend services for managing sensor data, user authentication, and system dashboards.

**Technologies**  
Django, Node.js, REST APIs, SQL databases

---

## Current Focus
- Scalable IoT system architectures
- Embedded–backend integration
- Applied mathematics for engineering systems
- Reliable and production-oriented software design

---

## GitHub Statistics

<!-- GitHub Stats -->
![GitHub Stats](https://github-readme-stats-git-master-odaris-projects.vercel.app/api?username=ODARI-CHARLES1&show_icons=true&include_all_commits=true&count_private=true&theme=tokyonight)

<!-- Top Languages -->
![Top Languages](https://github-readme-stats-git-master-odaris-projects.vercel.app/api/top-langs/?username=ODARI-CHARLES1&layout=compact&langs_count=6&theme=tokyonight)

<!-- GitHub Streak -->
![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=ODARI-CHARLES1&theme=tokyonight)

<!-- Profile Views -->
![Profile Views](https://komarev.com/ghpvc/?username=ODARI-CHARLES1&color=blueviolet&style=flat-square)

<!-- GitHub Trophies -->
![GitHub Trophies](https://github-profile-trophy.vercel.app/?username=ODARI-CHARLES1&theme=tokyonight&no-frame=false&no-bg=false&margin-w=5)
---

## Contact and Links

[![GitHub](https://img.shields.io/badge/GitHub-ODARI--CHARLES1-181717?style=for-the-badge&logo=github)](https://github.com/ODARI-CHARLES1)
[![Portfolio](https://img.shields.io/badge/Portfolio-View-blue?style=for-the-badge&logo=google-chrome)](https://charles.k.odari.portfolio.thegtm.or.ke/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin)](https://ke.linkedin.com/in/odari-kibisi-charles-329b19331)
[![Email](https://img.shields.io/badge/Email-Contact-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:daymondodari68@gmail.com)

---

> Engineering is the discipline of turning theoretical knowledge into reliable systems.
